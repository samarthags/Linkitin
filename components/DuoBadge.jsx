...
export default function DuoBadge({ duo }) {
  ...
  const totalForLevel   = duo.daysBonded + (duo.daysToNextLevel || 0);
  const progressPct     = duo.daysToNextLevel == null
    ? 100
    : Math.min(100, Math.max(6, (duo.daysBonded / Math.max(1, totalForLevel)) * 100));
        ...
        >
          ...
          >
            ...
            <div style={{ textAlign: "center", margin: "8px 0 4px" }}>
              <span style={{ fontSize: 46, fontWeight: 900, color: BLACK, lineHeight: 1 }}>{duo.level}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: "#8a8a80", marginLeft: 6 }}>/ 100</span>
            </div>

            {duo.daysToNextLevel != null ? (
              <div style={{ margin: "12px 0 18px" }}>
                ...
                <div style={{ fontSize: 11, color: "#8a8a80", marginTop: 6, textAlign: "center", fontWeight: 700 }}>
                  {duo.daysToNextLevel} day{duo.daysToNextLevel === 1 ? "" : "s"} to level {duo.level + 1}
                </div>
              </div>
            ) : (
            ...
            <div style={{ fontSize: 12, color: "#555", textAlign: "center", marginBottom: 18, fontWeight: 600 }}>
              Bonded {duo.daysBonded} day{duo.daysBonded === 1 ? "" : "s"} · 🔥 {duo.streak?.current || 0} day streak
            </div>
